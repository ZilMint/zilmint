...
if("text" in req.body.message && req.body.message.text == "/start"){
     const url = "https://api.telegram.org/bot<token>/sendMessage?chat_id="+req.body.message.chat.id+"&text=Send the image you want minted and add a caption in the following format. \n\nZilliqaAddress : title : description";
     request.get(url);
}
     ...
     if ("caption" in req.body.message){
          ...
          if(match.length != 3){
               ...
               const url = "<metadata-upload-api-url>?chat_id="+req.body.message.chat.id+"&file_id="+file_id+"&address="+address+"&title="+title+"&description="+description+"";
               request.get(url);
          }
     }
     else
     ...
     const url = "https://api.telegram.org/bot<token>/sendMessage?chat_id="+req.body.message.chat.id+"&text=Missing media. Send the image you want minted and add a caption in the following format. \n\nZilliqaAddress : title : description";
     request.get(url);
}
